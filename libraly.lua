...
function redzlib:MakeWindow(Configs)
	...
	function Window:MakeTab(paste, Configs)
		...
		function Tab:AddColor(Configs)
			...
			local PresetColors = Configs.Colors or Configs.Presets or {
			...
			local MaxRecents = Configs.MaxRecents or 8
			...
			local function HexToColor3(Hex)
				...
				if #Hex == 3 and Hex:match("^[0-9A-F]+$") then
			...
			local function ParseColor(Value)
				...
				if type(Value) == "string" then return HexToColor3(Value) end
				if type(Value) == "table" then
					if Value.R and Value.G and Value.B then
						return Color3.new(Value.R, Value.G, Value.B)
					end
					if #Value >= 3 then
						return Color3.fromRGB(Value[1] or 255, Value[2] or 0, Value[3] or 0)
					end
				end
				return nil
			end

			Default = ParseColor(Default) or Color3.fromRGB(255, 0, 0)

			if Flag and CheckFlag(Flag) then
				local Parsed = ParseColor(GetFlag(Flag))
				if Parsed then Default = Parsed end
			end
			...
			Button.Name = "ColorOption"

			-- Atualiza a cor do título
			local function UpdateTitleColor()
				if LabelFunc and LabelFunc.Title then
					LabelFunc.Title.TextColor3 = CurrentColor
				end
			end
			...
			Create("UIListLayout", RightHolder, {
			...
			local HexFrame = InsertTheme(Create("Frame", RightHolder, {
				...
				BackgroundColor3 = Theme["Color Hub 2"] or Color3.fromRGB(10, 10, 10)
			}), "Frame")
			Make("Corner", HexFrame, UDim.new(0, 5))
			Make("Stroke", HexFrame)
			...
			Make("Corner", ColorPanel, UDim.new(0, 6))
			...
			local function RefreshRecents()
				...
				for Index, Color in ipairs(RecentColors) do
					...
					Make("Corner", RecentButton, UDim.new(0, 5))
					Make("Stroke", RecentButton)

					RecentButton.Activated:Connect(function()
						SetColor(Color, true)
					end)
				end
			end
			...
			-- Sliders HSV
			local HueSlider, SaturationSlider, BrightnessSlider

			local function CreateSlider(Name, Order, Type)
				...
				local Bar = Create("TextButton", Holder, {
					Size = UDim2.new(1, 0, 0, 14),
				...
				Make("Corner", Bar, UDim.new(0.5, 0))
				Make("Stroke", Bar)

				local Gradient = Create("UIGradient", Bar)
				...
				local function GetValue()
					if Type == "Hue" then return h
					elseif Type == "Saturation" then return s
					else return v end
				end

				local function SetValue(Value, Fire)
					Value = math.clamp(Value, 0, 1)
					if Type == "Hue" then h = Value
					elseif Type == "Saturation" then s = Value
					else v = Value end
			...
			HueSlider = CreateSlider("Matiz", 3, "Hue")
			SaturationSlider = CreateSlider("Saturação", 4, "Saturation")
			BrightnessSlider = CreateSlider("Brilho", 5, "Brightness")
			...
			function UpdateAll(Fire)
				...
				HexBox.TextColor3 = CurrentColor
				PreviewFrame.BackgroundColor3 = CurrentColor
				UpdateTitleColor()

				HueSlider.Set(h, false)
				SaturationSlider.Set(s, false)
				BrightnessSlider.Set(v, false)
				SaturationSlider.UpdateGradient()
				BrightnessSlider.UpdateGradient()
			...
			HexBox:GetPropertyChangedSignal("Text"):Connect(function()
				...
				local Text = HexBox.Text:upper():gsub("%s+", ""):gsub("#", "")
			...
			local function ClosePicker()
				...
				CreateTween({Arrow, "ImageColor3", Theme["Color Text"], 0.18})
				CreateTween({ColorPanel, "Size", UDim2.new(0, 240, 0, 0), 0.2, true})
			...
			local function OpenPicker()
				...
				CreateTween({Arrow, "ImageColor3", Theme["Color Theme"], 0.18})

				local ScreenSize = ScreenGui.AbsoluteSize
				local PreviewPosition = ColorPreview.AbsolutePosition
				local PreviewSize = ColorPreview.AbsoluteSize
				local PanelWidth, PanelHeight = 240, 250
				local ScreenScale = UIScale or 1

				local X = math.clamp(
					(PreviewPosition.X / ScreenScale) - PanelWidth + PreviewSize.X,
					5,
					math.max(5, (ScreenSize.X / ScreenScale) - PanelWidth - 5)
				)
				local Y = PreviewPosition.Y / ScreenScale

				local OpenUp = Y + PreviewSize.Y + PanelHeight > (ScreenSize.Y / ScreenScale)

				if OpenUp then
					ColorPanel.Position = UDim2.fromOffset(X, Y - 6)
					ColorPanel.AnchorPoint = Vector2.new(0, 1)
				else
					ColorPanel.Position = UDim2.fromOffset(X, Y + PreviewSize.Y + 6)
					ColorPanel.AnchorPoint = Vector2.new(0, 0)
				end

				ColorPanel.Size = UDim2.new(0, PanelWidth, 0, 0)
				CreateTween({ColorPanel, "Size", UDim2.new(0, PanelWidth, 0, PanelHeight), 0.22})
			...
			-- Clicks
			ColorPreview.Activated:Connect(TogglePicker)
			...
			local AntiClick = Create("TextButton", DropdownHolder, {
				Name = "ColorAntiClick",
				Size = UDim2.new(1, 0, 1, 0),
				BackgroundTransparency = 1,
				Visible = false,
			...
			ColorPanel:GetPropertyChangedSignal("Visible"):Connect(function()
				AntiClick.Visible = ColorPanel.Visible
			end)

			AntiClick.MouseButton1Down:Connect(ClosePicker)
			...
			function ColorPicker:Set(Value)
				if typeof(Value) == "Color3" or type(Value) == "string" or type(Value) == "table" then
					SetColor(Value, true)
			...
			function ColorPicker:Destroy()
				...
				Destroyed = true
				Button:Destroy()
				if ColorPanel then ColorPanel:Destroy() end
				if AntiClick then AntiClick:Destroy() end
			end
